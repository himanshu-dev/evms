...
namespace MvcApplication1.Controllers
{
    ...
    public class TAVoucherController : Controller
    {
        int id;
        private VoucherEntities db = new VoucherEntities();

        [HttpPost]
        public ActionResult Create(TAVoucher tavoucher)
        {
            if (ModelState.IsValid)
            {
                ...
                {
                    setId();
                    tavoucher.VoucherID = this.id;
                    tavoucher.SubmittedBy = RegistrationController.UserName;
                    tavoucher.SubmitDate = DateTime.Now;
                    db.TAVouchers.Add(tavoucher);

                    var newVoucher = db.Vouchers.Create();
                    newVoucher.VoucherID = id;
                    newVoucher.VoucherType = "TAV";
